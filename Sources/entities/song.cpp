...
Song::Song() : id(0), releaseYear(0), duration(0), artistId(0), albumId(0) {}
...
void Song::setTitle(const std::string& newTitle) { title = newTitle; }
void Song::setGenre(const std::string& newGenre) { genre = newGenre; }
void Song::setReleaseYear(int year) { releaseYear = year; }
void Song::setDuration(int newDuration) { duration = newDuration; }
void Song::setAlbumId(int newAlbumId) { albumId = newAlbumId; }
void Song::setFilePath(const std::string& path) { filePath = path; }

bool Song::isSingle() const { return albumId == 0; }
